...
template PoseidonEx(nInputs, nOuts) {
    ...
    // Using recommended parameters from Poseidon1 whitepaper for the most part except for some of the following parameters from the Poseidon2 whitepaper https://eprint.iacr.org/2019/458.pdf: rounds are same P (size of the field) is greater than 2030, t are natural numbers with the special case of t', if t is small there's no difference between internal and external matrices
